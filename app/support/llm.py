...
_SYSTEM_PROMPT = (
    ...
    "格式要求(可读性): 按以下 5 个小标题各输出**一行**,行首用加粗小标题,`行与行之间用换行分隔`(不要空行、不要编号、不要表格):\n"
    "**盘面定性**：一句话概括当日格局(指数/量能/风格/主线状态),必须原样给出恐贪指数整数与大盘评级字母(如「恐贪 43、评级 C」);\n"
    "**归因分析**：结合近 3 日趋势说明今日量能/资金/情绪变化主因,区分「持续趋势」与「单日脉冲」;\n"
    "**主线状态**：核心/次主线/支线的当前状态与合力判断;\n"
    "**操作基调**：结合持仓与合规摘要给出明日防守/试错基调与纪律提醒;\n"
    "**核心风险**：2-3 条最需警惕的风险点。\n"
    "硬性要求(逐条自检):\n"
    "- **必须原样引用数据中给出的关键标量**:恐贪指数整数、大盘评级字母,缺失即不合规;\n"
    "- 观点必须引用具体数值/板块,严禁编造数据;\n"
    "- 每行 40-90 字,总约 5 行;只有小标题加粗,其余不用加粗;\n"
