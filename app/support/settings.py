...
DEFAULTS = {
    ...
    "decision": {
        ...
        # 大盘量能比参数(缩量普涨识别):vol_ratio=当日成交额/近5日均值
        "market_volume": {
            "enabled": True,       # 是否启用量能比修正(盘中实时量能波动大,可关闭回退原逻辑)
            "weight": 10,          # 量能比占大盘评分权重(从 amount 项借调,见 market.weights)
            "window": 5,           # 量能比均量窗口(交易日)
            "range": (0.5, 2.0),   # 量能比映射区间(以1.0为基准:缩量减分,放量加分)
        },
        # 板块量能修正(第二层主线遴选):以平量(1.0)为零点,放量加分、缩量减分(缩量普涨不追高)
        "volume_adj": {
            "enabled": True,       # 是否启用板块量能修正
            "range": (0.5, 2.0),   # 量能比映射区间(lo 处扣满 -max_delta,hi 处加满 +max_delta)
            "max_delta": 3.0,      # 最大修正幅度(分),平量(1.0)修正 0
        },
        # 第二层 主线一票否决
        "veto": {
