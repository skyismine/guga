...
- `build_snapshot(ctx)`: 报告开头浓缩 4 项核心结论(大盘定性/核心主线/仓位上限/核心风险),
...
def build_snapshot(ctx: dict) -> list:
    """生成「30秒速览」结构化 items(严格 4 条核心结论, 不堆砌具体数据)。"""
    mkt = ctx.get("market") or {}
    items = []
    items.append({"t": f"**大盘定性**:{_cell(mkt.get('qualify', '数据暂缺'))}"
                       f"(量能 {_cell(mkt.get('vol_tag'))} · 情绪 {_cell(mkt.get('emotion_tag'))})"})
    items.append({"t": f"**核心主线**:{_cell(ctx.get('core_names')) or '暂无明确主线'}"
                       f",强度 **{_cell(ctx.get('strength'))}**"})
    items.append({"t": f"**仓位上限**:{_cell(ctx.get('position_cap'))}"})
...
def plain_summary(ctx: dict, d: dict = None) -> str:
    ...
    lines = [
        ...
        f"仓位上限: {_cell(ctx.get('position_cap'))}",
